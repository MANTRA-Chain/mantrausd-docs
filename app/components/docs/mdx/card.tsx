...
// Map common Mintlify-style icon names (kebab-case) to lucide-react icons.
...
function renderLucideIcon(name: string): React.ReactNode | null {
  const Icon = lucideIconMap[name]
  return Icon ? <Icon className="w-5 h-5" strokeWidth={1.75} /> : null
}
...
export function Card({ title, icon, image, href, children }: CardProps) {
  const IconComponent = icon ? iconMap[icon] ?? renderLucideIcon(icon) : null
